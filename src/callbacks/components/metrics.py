...
class GleasonMetric:
    def __init__(
        ...
        self.metric_collection = get_metrics(
            mode + f"_{GS_name}" + core_prename, num_classes
        ).clone(prefix)
        if GS_name == "ALL":
            self.metric_collection = get_metrics(
                mode + core_prename, num_classes, use_all_metrics=True
            ).clone(prefix)
    ...
    def update(self, set: Literal["val", "test"], logits, labels, grades):
        ...
        if set is "val":
            ...
            for i, (group_name, group_state) in enumerate(self.gs_groups.items()):
                indx = np.isin(grades[0, :], group_state[:, 0]) & np.isin(
                    grades[1, :], group_state[:, 1]
                )
