...
from ..preprocessing import split_into_patches_pixels
import os
from itertools import product
import numpy as np


# class PatchAccessor(ABC):
#    @property
#    @abstractmethod
...
class CachedImageGrid(SubPatchAccessorMixin):
    def __init__(self, img, patch_size_pixels, root):
        grid = split_into_patches_pixels(img, *patch_size_pixels)
        self._shape = grid.shape

        self._cache = Cache()
        self._hits = np.zeros(self.shape).astype("bool")
    ...
    @property
    def subpatch_shape(self):
        return self._shape[2:]

    def get_subpatch(self, i, j):

        i = int(i)
        j = int(j)

        if not self._hits[i, j]:
            self._cache[(i, j)] = self._cache["grid"][i, j]
            self._hits[i, j] = True

            if np.all(self._hits):
                del self._cache["grid"]
...
#
# class ImagePatchGrid(PatchAccessor):
#    def __init__(
#        self,
#        path_to_image,
#        patch_height_pixels,
#        patch_width_pixels,
#        discard_out_of_grid_patches_from="bottomleft",
#    ):
#        """Class for dynamically loading patches of an image using slice notation"""
#
#        self.path_to_image = path_to_image
#
#        self.patch_height_pixels = patch_height_pixels
#        self.patch_width_pixels = patch_width_pixels
#        self.discard_out_of_grid_patches_from = discard_out_of_grid_patches_from
#
...
#    @property
#    def img(self):
#        return np.load(self.path_to_image)
#
#    def get_slice(self, i1, i2, j1, j2):
#
#        dw = self.patch_width_pixels
...
#        h = int(self.img_shape[0] / dh)
#
#        discard_from = self.discard_out_of_grid_patches_from
#
#        origin = [0, 0]
#        if "bottom" in discard_from:
#            origin[0] = 0
