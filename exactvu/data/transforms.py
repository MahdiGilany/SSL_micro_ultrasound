...
class TransformConfig:
    ...
    tensor_augs_config: Optional[TensorAugsConfig] = None
    us_augs_config: Optional[UltrasoundAugsConfig] = None


class Normalize:
    def __init__(
        self,
        mode: Literal["instance", "global"],
        type: Literal["z-score", "min-max"],
        truncate: bool = True,
    ):
        """
...
class TensorImageAugmentation:
    def __init__(
        ...
        if random_erasing:
            augs.append(
                T.RandomErasing(p=prob, scale=(0.02, 0.07), ratio=(0.3, 3.3), value=0.5)
            )
        if random_invert:
