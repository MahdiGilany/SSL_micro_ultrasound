...
from typing import Any, Callable, Union, overload, NewType
...
class Splits:
    def __init__(self, config: SplitsConfig):

        self._splits = _get_splits_impl(
            config.cohort_specifier,
            config.resample_train_val,
            config.split_seed,
            config.train_val_ratio,
            config.balance_classes,
            config.benign_cores_selection_seed,
        )
    ...
    def get_train(
        self, merge_centers=False
    ) -> Union[CoresList, Mapping[str, CoresList]]:
        return self._get_splits_impl(idx=0, merge_centers=merge_centers)

    def get_val(self, merge_centers=False) -> Union[CoresList, Mapping[str, CoresList]]:
        return self._get_splits_impl(idx=1, merge_centers=merge_centers)
...
def select_patient_splits_with_specified_cancer_core_ratio(
    ...
    while True:
        ...
        if abs(test_ratio - target_cancer_core_test_size) <= TOLERANCE:
            ...
            logging.info(f"It took {n_iters} iterations to obtain the desired splits")
...
def select_cohort(
    ...
    train_cores = list(np.sort(train_cores["core_specifier"].values))
    val_cores = list(np.sort(val_cores["core_specifier"].values))
    test_cores = list(np.sort(test_cores["core_specifier"].values))
...
def get_crceo_428_splits(resample_train_val, split_seed, train_val_ratio=0.25):
    ...
    from exactvu.resources import metadata, crceo_428_splits
...
def _get_splits_impl(
    ...
    if cohort_specifier == "all":
        ...
        if isinstance(cohort_specifier, list):
            return {
                ...
                for _cohort_specifier in cohort_specifier
            }
...
class HasProstateMaskFilter:
    def __init__(self):
        from exactvu.client import paths_to_prostate_masks

        self.patches_with_masks = list(paths_to_prostate_masks().keys())
